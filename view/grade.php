            	...
            	<!-- general form elements -->
              	<div class="box box-primary">
                	<div class="box-header with-border">
                  		<h3 class="box-title">Añadir Grado</h3>
                	</div><!-- /.box-header -->
                	...
                	<form role="form" action="../index.php" method="post" id="form1">                    
                  		<div class="box-body">
                    		<div class="form-group" id="divGrade">
                      			...
                      			<input type="text" class="form-control" id="name" placeholder="Inserte el Nombre" name="name" autocomplete="off">
                    		</div>
                            <div class="form-group" id="divAdmissionFee">
                      			<label for="">Tarifa de Admisión($)</label>
                      			<input type="text" class="form-control" id="admission_fee" placeholder="Inserte una Tarifa de Admisión" name="admission_fee" autocomplete="off">
                    		</div>
                             <div class="form-group" id="divHallCharge">
                      			<label for="">Carga del Salon(%)</label>
                      			<input type="text" class="form-control" id="hall_charge" placeholder="Inserte la Carga Maxima del Salon" name="hall_charge" autocomplete="off">
                    		</div>
                  		</div><!-- /.box-body -->
                  		<div class="box-footer">
                  			<input type="hidden" name="do" value="add_grade" />
                    		<button type="submit" class="btn btn-primary" id="btnSubmit">Siguiente</button>
                  		</div>
                	</form>
				</div><!-- /.box -->
			</div>
                            ...
                            <div class="panel-heading">               
                             ...
                             <form role="form" action="../index.php" method="post" id="form2">
                                ...
                                <div class="panel-footer bg-blue-active">
                                    ...
                                    <button type="submit" onClick="" class="btn btn-info btnS" id="btnSubmit1" style="width:100%;">Enviar</button>
                                </div>
                            </form>       
                        </div><!--/.panel-->
                    </div><!--/.col-md-3 --> 
...
//MSK-000098-form submit
	...
	if(name == ''){
		...
		$('#divGrade').append('<span id="spanName" class="glyphicon glyphicon-remove form-control-feedback set-width-tooltip" data-toggle="tooltip"    title="El Grado es obligatorio" ></span>');	
	...
	if(admission_fee == ''){
		...
		$('#divAdmissionFee').append('<span id="spanAdmissionFee" class="glyphicon glyphicon-remove form-control-feedback set-width-tooltip" data-toggle="tooltip"    title="La Tarifa de Admisión es obligatoria" ></span>');	
	...
	if(hall_charge == ''){
		...
		$('#divHallCharge').append('<span id="spanHallCharge" class="glyphicon glyphicon-remove form-control-feedback set-width-tooltip" data-toggle="tooltip"    title="La Carga del Salon es obligatoria" ></span>');	
...
$("#form2").submit(function (e) {
	...
	for(i=1; i<rowCount+1; i++){
		...
		if(markRange==""){
			...
			$("#range_td_"+i).append('<span id="spanMarkRange" class="glyphicon glyphicon-remove form-control-feedback set-color-tooltip" data-toggle="tooltip" title="El Rango de Calificación es obligatorio" ></span>');
		...
		if(markGrade==""){
			...
			$('#grade_td_'+i).append('<span id="spanMarkGrade" class="glyphicon glyphicon-remove form-control-feedback set-color-tooltip" data-toggle="tooltip" title="El Rango de Calificación es obligatorio" ></span>');
			...
			<div class="col-md-7">
            	<div class="box">
                	<div class="box-header">
                  		<h3 class="box-title">Todos los Grados</h3>
                	</div><!-- /.box-header -->
                	<div class="box-body table-responsive">
                		...
                		<table id="example1" class="table table-bordered table-striped">
                    		<thead>
                        		<th>ID</th>
                            	<th>Grado</th>
                                <th>Tarifa de Admisión($)</th>
                                <th>Carga del Salón(%)</th>
                            	<th>Acción</th>
                        	</thead>
                        	<tbody>
<?php
...
if($cant_remove1 > 0 || $cant_remove2 > 0 || $cant_remove3 > 0 || $cant_remove4 > 0 || $cant_remove5 > 0 || $cant_remove6 > 0 || $cant_remove7 > 0 ){
	...
	echo ' <a href="#modalMRangeGrade" onClick="showModal1(this)" class="btn btn-warning btn-xs" data-id="'.$id.'" data-toggle="modal">Ver Rango de Calificación</a>';
           			...
           			<div class="col-md-6">
                		<div class="panel">
        					<div class="panel-heading bg-orange">              
        						<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button>
          						<h4 class="modal-title custom_align" id="Heading">Editar Grado</h4>
   							</div>
                            <div class="panel-body"> <!-- Start of modal body--> 
                                ...
                                <div class="form-group" id="divHChargeUpdate">
                                    <label for="">Carga del Salón(%)</label>
                                    <input class="form-control" type="text" id="hall_charge1" name="hall_charge" autocomplete="off">
                                </div>
                            </div><!--/.modal body-->
                            <div class="panel-footer bg-gray-light">
                                <input type="hidden" name="id" id="id" value="">
                                <button type="button" onClick="Updategrade(this)" id="btnSubmit1" class="btn btn-info" style="width: 100%;"><span class="glyphicon glyphicon-ok-sign"></span> Actualizar</button>    
                            </div><!--/.panel-footer--> 
            			</div><!--/.panel-->
            		</div><!--/.col-md-6-->
            	</div><!--/.row-->                                        
...
//MSK-000108
	...
	if(name1 == ''){
		...
		$('#divGradeUpdate').append('<span id="spanNameUpdate" class="glyphicon glyphicon-remove form-control-feedback set-width-tooltip" data-toggle="tooltip"    title="El Grado es obligatorio" ></span>');	
	...
	if(admission_fee1 == ''){
		...
		$('#divAFeeUpdate').append('<span id="spanAFeeUpdate" class="glyphicon glyphicon-remove form-control-feedback set-width-tooltip" data-toggle="tooltip"    title="La Tarifa de Admisión es obligatoria" ></span>');	
	...
	if(hall_charge1 == ''){
		...
		$('#divHChargeUpdate').append('<span id="spanHChargeUpdate" class="glyphicon glyphicon-remove form-control-feedback set-width-tooltip" data-toggle="tooltip"    title="La Carga del Salón es Requerida" ></span>');	
    		...
    		<div class="modal-content">
				...
				<div class="modal-body bgColorWhite">
      			...
      			<div class="modal-footer">
					<a href="#" style='margin-left:10px;' id="btnYes" class="deletebtn btn btn-danger col-sm-2 pull-right">Si</a><!-- MSK-000125 -->
	...
	<div class="modal msk-fade" id="deleteConfirmRG" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  		<div class="modal-dialog">
    		<div class="modal-content">
				<div class="modal-header bg-primary">
        			<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
        			<h4 class="modal-title" id="frm_title">Eliminar1</h4>
      			</div>
				<div class="modal-body bgColorWhite">
        			<strong style="color:red;">¿Estas Seguro?</strong>  Quieres Eliminar este Registro
        		</div>
      			<div class="modal-footer">
					<a href="#" style="margin-left:10px;" id="btnYesRG" class="deletebtn btn btn-danger col-sm-2 pull-right">Si</a><!-- MSK-000125 -->
        			<button type="button" class="btn btn-primary col-sm-2 pull-right" data-dismiss="modal" id="frm_cancel">No</button>
      			</div>
    		</div>
  		</div>
